...
- SplinePlug : Fixed bugs affecting default values. All child plugs are now at their default values following construction and following a call to either `setToDefault()` or `resetDefault()`.
...
- Constraints : Behaviour if the target location didn't exist was previously undefined, and often an error. This behaviour has now been specified - it is now consistently an error, unless you set the new `ignoreMissingTarget` plug, in which case the constraint will just do nothing. Also, a target of "" is now a no-op, and you must explicitly enter "/" in order to constrain to the scene root.
