...
- RenderPassEditor :
  ...
  - The default render pass type configurations include :
    ...
    - A "reflection" type for catching reflections cast by `caster` locations on `catcher` locations. The roughness of the reflective material can be configured per catcher location by creating a `user:reflectionCatcher:roughness` float attribute.
    - A "reflectionAlpha" type for catching reflection mattes cast by `caster` locations on `catcher` locations. The colour of the matte can be configured per caster location by creating a `user:reflectionCaster:color` Color3f attribute.
